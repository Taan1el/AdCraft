...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/analyze" && req.method === "POST") {
      ...
      if (typeof adTypeRaw !== "string" || !ALLOWED_AD_TYPES.has(adTypeRaw as AdType)) {
        return withCors(
          req,
          env,
      ...
      const adType = adTypeRaw as AdType;
      ...
      if (bytes === 0) {
        return withCors(req, env, json({ error: "Uploaded file is empty" }, { status: 400 }));
      }
      const imageType = typeof f.type === "string" ? f.type.trim().toLowerCase() : "";
